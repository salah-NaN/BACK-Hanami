...
export default router
    .get("/clientes", checkToken, async (req, res) => await readItems(req, res, Clientes))
    .get("/clientes/:id", checkToken, async (req, res) => await readItem(req, res, Clientes))
    .post("/clientes", checkToken, async (req, res) => await createItem(req, res, Clientes))
    .put("/clientes/:id", checkToken, async (req, res) => await updateItem(req, res, Clientes))
    .delete("/clientes/:id", checkToken, async (req, res) => await deleteItem(req, res, Clientes))
    ...
    .post("/clientes/register", async (req, res) => {
        try {
            ...
            const posibleCliente = await Clientes.findOne({ where: { email } }, {raw: true})
            if (posibleCliente) return res.status(400).json({ message: 'Usuario registrado' })
    ...
    .post("/clientes/login", async (req, res) => {
        try {
            ...
            const token = jwt.sign({ cliente_id: cliente.id, cliente_nombre: cliente.nombre }, secretKey, { expiresIn: '2h' })
